import {PostCreateDTO, UpdateCreateDTO} from "@/types/post";
...
export const updatePost = async (createPostDTO: UpdateCreateDTO) => {
    const response = await fetch(`${POST_ENDPOINT}/${createPostDTO.id}`, {
        method: 'PATCH',
        headers: {
            'Content-Type': 'application/json'
        },
